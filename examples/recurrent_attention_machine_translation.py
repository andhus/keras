...
class DenseAnnotationAttention(Layer):
    """Recurrent attention mechanism for attending sequences.

    This Layer implements the attention mechanism used in [1] for machine
    translation. It is, however, a generic sequence attention mechanism that can be
    used for other sequence-to-sequence problems.

    It should be used in conjunction with a wrapped (non attentive) RNN Cell, such as
    ...
    def __init__(self, cell,
                 ...
                 **kwargs):
        super(DenseAnnotationAttention, self).__init__(**kwargs)
        self.cell = cell
    ...
    def call(self, inputs, states, constants, training=None):
        ...
        attention_h = self.attention_call(
            cell_states=cell_states,
            attended=attended,
            attended_mask=attended_mask)

        cell_input = concatenate([inputs, attention_h])
        ...
        output = concatenate([cell_output, attention_h])
    ...
    def build(self, input_shape):
        ...
        if hasattr(self.cell, "output_size"):
            cell_output_size = self.cell.output_size
        else:
            cell_output_size = cell_state_size[0]
        self.output_size = cell_output_size + self.attention_size

        self.built = True

    def attention_build(self, cell_state_size, attended_shape):
        ...
        self.attention_size = attended_shape[-1]
        units = u_shape[-1]
    ...
    def trainable_weights(self):
        return (super(DenseAnnotationAttention, self).trainable_weights +
                self.cell.trainable_weights)

    @property
    def non_trainable_weights(self):
        return (super(DenseAnnotationAttention, self).non_trainable_weights +
...
if __name__ == '__main__':
    ...
    initial_state = Dense(RECURRENT_UNITS, activation='tanh')(h_enc_bkw_final)
